...
import {initializeImageMagick, ImageMagick, MagickGeometry, } from "@imagemagick/magick-wasm";
  ...
  const handleWidthChange = (val: string) => {
    ...
    if (!isNaN(newWidth)) {
        ...
        if (lockAspectRatio && originalDimensions) {
            ...
            // Update percentage
            const newPercentage = Math.round((newWidth / originalDimensions.width) * 100);
  ...
  const handleHeightChange = (val: string) => {
    ...
    if (!isNaN(newHeight)) {
        ...
        if (lockAspectRatio && originalDimensions) {
            ...
            // Update percentage
            const newPercentage = Math.round((newHeight / originalDimensions.height) * 100);
            setPercentage(newPercentage);
        } else if (originalDimensions && !lockAspectRatio) {
             const newPercentage = Math.round((newHeight / originalDimensions.height) * 100);
             setPercentage(newPercentage);
        ...
        <div className="flex flex-col gap-6 w-full py-4 md:order-2">
            ...
            <div className="flex items-center space-x-2">
                <Checkbox
                    ...
                    disabled={files.length === 0}
                />
                <Label htmlFor="lock-ratio" className="cursor-pointer">Lock Aspect Ratio</Label>
            </div>
