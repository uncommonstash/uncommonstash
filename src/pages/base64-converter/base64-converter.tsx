...
export default csr(function Base64ConverterPage() {
  ...
  const encode = useCallback((str: string, variant: Variant, usePadding: boolean) => {
    try {
          ...
          <CardHeader>
            <div className="flex justify-between items-start">
              <div>
                ...
                <CardDescription>
                  Encode or decode text with support for URL-safe variants and padding control.
                </CardDescription>
              </div>
              <Tabs value={mode} onValueChange={(v) => handleModeChange(v as "encode" | "decode")}>
                <TabsList className="h-9">
                  <TabsTrigger value="encode" className="h-7">Encode</TabsTrigger>
                  <TabsTrigger value="decode" className="h-7">Decode</TabsTrigger>
                </TabsList>
              </Tabs>
            </div>
          </CardHeader>
          <CardContent className="flex-1 flex flex-col gap-6 min-h-0 overflow-auto">
            <div className="flex flex-wrap items-center gap-6">
              <div className="flex items-center space-x-2">
                ...
                <Tabs value={variant} onValueChange={(v) => setVariant(v as Variant)}>
                  <TabsList className="h-8">
                    <TabsTrigger value="standard" className="h-6 text-xs px-2">Standard</TabsTrigger>
                    <TabsTrigger value="url-safe" className="h-6 text-xs px-2">URL Safe</TabsTrigger>
                  </TabsList>
                </Tabs>
              </div>
              ...
              {mode === "encode" && (
                <div className="flex items-center space-x-2">
                  ...
                  />
                  <Label htmlFor="padding-toggle" className="text-sm font-medium">Padding (=)</Label>
                </div>
              )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 flex-1 min-h-0">
              <div className="flex flex-col gap-2 h-full">
                <Label htmlFor="input" className="text-xs text-muted-foreground uppercase tracking-wider font-bold">
              ...
              <div className="flex flex-col gap-2 h-full relative">
                <Label htmlFor="output" className="text-xs text-muted-foreground uppercase tracking-wider font-bold">
