...
export function BatteryOverview({ battery }: { battery: BatteryPlistData }) {
  ...
  return <div className="space-y-10"><section className="space-y-2"><div><h2 className="text-base font-semibold">Battery level from Battery UI</h2><p className="text-xs text-muted-foreground">Curve and app roster source: BatteryUISysdiagnose.plist. Drag across the curve to query a range.</p></div><BatteryChart points={battery.points} selectedRange={state.range} onRangeChange={setRange} /></section>{failure ? <p className="text-sm text-destructive">{failure}</p> : <AppTable apps={apps} energyRows={energy.state === "ready" ? energy.result.rows.filter((row): row is import("@/workers/sysdiagnose-query/query.protocol").AppEnergyAttributionRow => "consumerNode" in row) : []} runtimeRows={runtime.state === "ready" ? runtime.result.rows.filter((row): row is import("@/workers/sysdiagnose-query/query.protocol").AppRuntimeRow => "bundleId" in row) : []} energyProvenance={energy.state === "ready" ? energy.result.provenance : undefined} runtimeProvenance={runtime.state === "ready" ? runtime.result.provenance : undefined} />}</div>;
}
