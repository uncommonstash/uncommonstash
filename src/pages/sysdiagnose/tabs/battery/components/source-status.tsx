...
export function SourceStatus({ provenance }: { provenance?: QueryProvenance }) {
  if (!provenance)
    return (
      <p className="text-xs text-muted-foreground">
        Waiting for Powerlog query results.
      </p>
    );
  const aggregation = provenance.aggregation
    .map(
      (entry) =>
        `${entry.operation}(${entry.input}) by ${entry.groupBy.join(", ")}`,
    )
    .join("; ");
  ...
  return (
    <p
      ...
      data-effective-start-ms={effective?.startMs}
      data-effective-end-ms={effective?.endMs}
    >
      Source: {provenance.source.table} · {aggregation || "source rows"} ·{" "}
      {provenance.source.clock}
      {effective ? (
        <>
          {" "}
          · Effective source range: {time(effective.startMs)}–
          {time(effective.endMs)}
        </>
      ) : (
        " · No overlapping source rows"
      )}
    </p>
  );
}
