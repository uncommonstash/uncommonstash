  ...
  const tokens = useMemo(() => {
    ...
    parts.forEach((part, index) => {
        ...
        if (part.startsWith('<')) {
            ...
            if (tagMatch) {
                result.push(<span key={index + "-1"} className="text-blue-600 dark:text-blue-400">{tagMatch[1]}</span>); // < or </
                result.push(<span key={index + "-2"} className="text-purple-600 dark:text-purple-400 font-bold">{tagMatch[2]}</span>); // tag name

                // Attributes
                if (tagMatch[3]) {
                     // Naive attribute highlighting
                     result.push(<span key={index + "-3"} className="text-orange-600 dark:text-orange-400">{tagMatch[3]}</span>);
                }

                result.push(<span key={index + "-4"} className="text-blue-600 dark:text-blue-400">{tagMatch[4]}</span>); // > or />
            } else {
                // Comment or malformed tag
                if (part.startsWith('<!--')) {
                     result.push(<span key={index} className="text-gray-500 italic">{part}</span>);
                } else {
                     result.push(<span key={index} className="text-blue-600 dark:text-blue-400">{part}</span>);
                }
            }
        } else {
            // Text content
            result.push(<span key={index} className="text-gray-800 dark:text-gray-200">{part}</span>);
        }
    });
        ...
        <div className="flex flex-col h-full min-h-0">
          <div className="flex justify-between items-center mb-2">
            ...
            <div className="flex gap-2">
                ...
                <Button onClick={handleMinify} size="sm" variant="outline">Minify</Button>
            </div>
          </div>
          <Textarea
            value={input}
