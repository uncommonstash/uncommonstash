...
function rows(sql: string): Array<Record<string, unknown>> {
  ...
  if (sql.includes("consumerNodeId"))
    return [
      {
        endSec: 8_300,
        intervalSec: 3_600,
        ...
        rootNodePermanent: 1,
        consumerNodeId: 2,
        consumerNodeName: "com.example.app",
        consumerNodePermanent: 0,
        rawEnergy: 75,
      },
    ];
  if (sql.includes("BundleID AS bundleId"))
    return [
      {
        endSec: 8_300,
        intervalSec: 3_600,
...
describe("Powerlog query SQL", () => {
  ...
  it("returns RootNodeID component totals, node identity, coverage gaps, and full overlapping intervals", () => {
    const result = executeQueryPlan(queryRows, {
      kind: "root-node-component-totals",
      range: { startMs: 8_000_000, endMs: 9_000_000 },
    });
    expect(result.rows).toEqual([
      {
        interval: { startMs: 5_700_000, endMs: 9_301_000 },
        rootNode: { id: 9, name: "CPU", isPermanent: true },
        rawEnergy: 100,
      },
    ]);
    expect(result.provenance.effectiveRange).toEqual({
      startMs: 5_700_000,
      endMs: 9_301_000,
    });
    expect(result.provenance.coverage).toHaveLength(2);
    expect(result.provenance.aggregation).toEqual([
      {
        operation: "sum",
        input: "Energy",
  ...
  it("preserves consumer and root identities for app attribution and source seconds for runtime", () => {
    ...
    const attribution = executeQueryPlan(queryRows, {
      kind: "app-energy-attribution",
      ...plan,
    });
    expect(attribution.rows[0]).toMatchObject({
      rawEnergy: 75,
      rootNode: { id: 9, name: "CPU", isPermanent: true },
      consumerNode: { id: 2, name: "com.example.app", isPermanent: false },
    });
    const runtime = executeQueryPlan(queryRows, {
      kind: "app-runtime",
      ...plan,
    });
  ...
  it("rejects a duplicate typed query result instead of silently shaping it", () => {
    const duplicate: QueryRows = (sql) =>
      sql.includes("timestamp AS monotonicSec")
        ? offsets
        : sql.includes("consumerNodeId") || sql.includes("BundleID AS bundleId")
          ? []
          : [
              {
                endSec: 8_300,
    ...
    expect(() =>
      executeQueryPlan(duplicate, {
        kind: "root-node-component-totals",
        range: { startMs: 0, endMs: 10_000_000 },
      }),
    ).toThrow("duplicate source result");
  });
});
