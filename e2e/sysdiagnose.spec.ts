...
test("sysdiagnose battery graph renders from PAX archive", async ({
  page,
}) => {
  ...
  await expect(
    page.getByText("Battery level over time"),
  ).toBeVisible();
  const svgPath = page.locator('svg[aria-label="Battery level over time"] path');
